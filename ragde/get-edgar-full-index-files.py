...
def download_index_files(min_year=MIN_YEAR, max_year=MAX_YEAR, quarters=[1, 2, 3, 4]):
    index_base = "https://www.sec.gov/Archives/edgar/full-index/%s/QTR%s/form.gz"
    download_dir = "%s/sec-downloads" % HOME_DIR
    for year in range(min_year, min_year + 1):
        for quarter in quarters:
            print("Downloading index file for %s, Q%s" % (year, quarter))
            try:
                download_fpath = "%s/form-%s-QTR%s.gz" % (download_dir, year, quarter)
...
def unzip_index_files(path_to_file, path_to_destination):
    ...
    return 0
download_index_files(2018,2018,[4])
...
def extract_10k_lines():
    ...
    for YYYY in range(MIN_YEAR, MAX_YEAR + 1):
        ...
        with open("%s/sec-company-index-files-combined/form-%s-10ks.txt" % (HOME_DIR, YYYY), 'w') as outfile:
            ...
            print
            "%s 10-K lines extracted" % len(outlines)
...
def combine():
    header = '\t'.join(['formtype', 'companyname', 'cik', 'filingdate', 'url']) + '\n'
    print
    "Combining all years' 10-K submissions data"
